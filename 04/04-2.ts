...
function solve(boards: Cell[][][], drawNumbers: number[]): number | void {
  for (const drawNumber of drawNumbers) {
    for (const board of boards) {
      ...
      if (isSolvedBoard(board) && areOtherBoardsSolved(boards, board)) {
...
function areOtherBoardsSolved(boards: Cell[][][], board: Cell[][]): boolean {
  for (const otherBoard of boards) {
    if (otherBoard === board) continue;
    if (!isSolvedBoard(otherBoard)) return false;
  }
  return true;
}
