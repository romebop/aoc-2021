...
function solve(scanners: Scanner[]): number {
  ...
  const scannerPoints: Point[] = [{ x: 0, y: 0, z: 0 }];
  scanners.splice(0, 1);
  ...
  return Math.max(...getManhattanDistances(scannerPoints));
}
...
function getManhattanDistances(points: Point[]): number[] {
  const manhattanDistances = [];
  for (let i = 0; i < points.length; i++) {
    for (let j = i + 1; j < points.length; j++) {
      manhattanDistances.push(getManhattanDistance(points[i], points[j]));
    }
  }
