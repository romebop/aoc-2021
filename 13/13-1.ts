...
console.log(solve(origami, folds.slice(0, 1)));
...
function solve(origami: string[][], folds: Fold[]): number {
  for (const fold of folds) {
    origami = foldOrigami(origami, fold);
  }
  return origami.reduce((totalDots, row) => (
    totalDots + row.reduce((rowDots, point) => (
      rowDots + (point === '#' ? 1 : 0)
    ), 0)
  ), 0);
}
